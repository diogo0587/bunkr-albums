...
import { Copy, File, FileWarning, ExternalLink, Play } from 'lucide-react';
...
function isPreviewable(url: string): boolean {
  const ext = url.split('?')[0].split('.').pop()?.toLowerCase() || '';
  return ['mp4', 'webm', 'mkv', 'avi', 'mov', 'mp3', 'wav', 'flac', 'ogg', 'jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext);
}
...
export function FileListItem({ file, selected, onSelect, onCopy, index }: FileListItemProps) {
  ...
  const canPreview = isDirectFile && isPreviewable(file.url);
  ...
  return (
    <>
      ...
      >
        ...
        {isDirectFile && isImageFile(file.url) && (
          <div className="w-10 h-10 rounded-md overflow-hidden bg-slate-700 flex-shrink-0 hidden sm:block">
            <img
              src={file.url}
              alt=""
        ...
        {canPreview && (
          <button
            ...
            className="p-1.5 text-slate-400 hover:text-cyan-400 transition-colors flex-shrink-0"
